...
- **物流配送管理**：審核配送申請、更新狀態（pending → delivering → arrived），物品送達車站後須由站務員核對旅客填寫的「驗證描述」並執行「核對驗證描述並結案」，才會將物品標記為已領回並記錄核對人員與時間，確保不是人到了就直接放行；配送列表將「物品原始描述」與「旅客驗證描述」並排顯示，不需另外點開即可直接比對核實；可取消重複/過期的申請；**配送限制規則**可在後台新增/編輯/刪除（不再寫死於程式碼）
...
未設定 `GEMINI_API_KEY` 時，AI 辨識、以圖找圖、許願池語意比對相關功能會回傳錯誤或自動降級為純關鍵字比對；未設定 Gmail 帳密時，email 通知會靜默跳過。
